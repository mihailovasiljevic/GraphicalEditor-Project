...
public class ElipseState extends State{
	...
	public ElipseState(CascadeDiagramView md) {
      	med = md;
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getButton()==MouseEvent.BUTTON1){
			 if (med.getDiagram().getDiagramModel().getElementAtPosition(position)==-1){
				 med.getCommandManager().addCommand(new AddDeviceCommand(med.getDiagram().getDiagramModel(),med.getDiagram().getSelectionModel(),position,CascadeDiagramView.ELIPSE));
			 }
				...
				if(selectionModel.getSelectionListSize() == 1){
					...
					Application.getInstance().getStatusBar().setStatus("Elipse State");
					Application.getInstance().getStatusBar().setElementType("Elipse");
				}
				else
